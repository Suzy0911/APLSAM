...
class MLAHead(nn.Module):
    ...
    def forward(self, mla_p2, mla_p3, mla_p4, mla_p5, scale_factor, gaussian_mask=None):
        mla_p2 = self.transformer(value=gaussian_mask, key=mla_p2, query=mla_p2)  # TODO
        mla_p3 = self.transformer(value=gaussian_mask, key=mla_p3, query=mla_p3)
        mla_p4 = self.transformer(value=gaussian_mask, key=mla_p4, query=mla_p4)
        mla_p5 = self.transformer(value=gaussian_mask, key=mla_p5, query=mla_p5)
...
class VIT_MLAHead_h(nn.Module):
    ...
    def forward(self, inputs, scale_factor1, scale_factor2, gaussian_mask):
        x = self.mlahead(inputs[0], inputs[1], inputs[2], inputs[3], scale_factor=scale_factor1, gaussian_mask=gaussian_mask)
