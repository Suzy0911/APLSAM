...
def set_save_path(save_path, remove=True):  # 在train_MAML中使用
...
def cal_dice_iou_ber(y_pred, y_true):
    batchsize = y_true.shape[0]
    y_pred, y_true = y_pred.permute(0, 2, 3, 1).squeeze(-1), y_true.permute(0, 2, 3, 1).squeeze(-1)
    with torch.no_grad():
        assert y_pred.shape == y_true.shape
        ...
        for i in range(batchsize):
            print('before')
            # thresholds = threshold_multiotsu(y_pred[i]*255, classes=2)
            true = y_true[i].flatten()
            pred = y_pred[i].flatten()
            # print(thresholds[-1])
            TP, TN, FP, FN, BER, ACC = get_binary_classification_metrics(pred * 255,
        ...
        return dice / batchsize, iou / batchsize, (pos_err + neg_err) / 2 / batchsize, np.array(0)
