...
def save_fig(tensor, name):
    ...
    img.save(os.path.join('save/imgs/LAGP', name + appendix))

def save_fig_pred(tensor, name):
    filtered_tensor = (tensor * 255) > 90
    filtered_tensor = filtered_tensor.float()
    numpy_array = (filtered_tensor * 255).byte().numpy()
    numpy_array = numpy_array.transpose(1, 2, 0)
    if numpy_array.shape[-1] == 1:
        numpy_array = np.squeeze(numpy_array)
        appendix = '.png'
...
def make_data_loaders(config):
    ...
    return support_train_loader, query_train_loader, support_eval_loader
...
def train(train_batch, model, loss_mode, epoch):
    ...
    support_mask = train_batch['gt'].to(device)  # ground truth
    max_val = support_mask.max()
    gt = (support_mask == max_val)  # important here
    gt = gt * support_mask
    model.optimizer.zero_grad()
    pred = model(inp, gt, support_mask, num_points=3)
    if loss_mode == 'bce':
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config',
                        default="/root/SAM_Adapter_MAML/configs/test_LARGE_new.yaml")  # /root/SAM_Adapter_MAML/configs/test.yaml
    parser.add_argument('--model',
                        default='/root/SAM_Adapter_MAML/save/_new_module/model_epoch_best_dice.pth')  # 权重保存路径/root/autodl-tmp/model_epoch_best_ber.pth
    ...
    support_train_loader, query_train_loader, support_eval_loader = make_data_loaders(
        config)  # 这里的eval是真正的evaluation_loader

    dice_recort = []
    total_data_len = len(os.listdir(os.path.join(data_path, task, 'query_train', 'img')))
    for i, (support_train_batch, query_train_batch) in enumerate(zip(support_train_loader, query_train_loader)):
        torch.cuda.empty_cache()
        if i % 2 == 0:
        ...
        if i % 2 == 0:
            ...
            for epoch in range(epochs):  # 每个one-shot样本更新几步
                ...
                loss_log = train(support_train_batch, model, loss_mode, epoch)  # 训练
                print(f'loss:{loss_log}')
                lr_scheduler.step()  # 余弦退火学习率
        metric1, metric2, metric3, metric4 = eval_psnr(support_train_batch, query_train_batch, model, i,
                                                       data_norm=config.get('data_norm'),
                                                       eval_type=config.get('eval_type'),
                                                       eval_bsize=config.get('eval_bsize'),
                                                       verbose=True)  # evaluation

        if metric1 >= 0.1:  # 否则是病人间错位support和query没匹配上
        ...
        if metric1 < 1:  # or metric1 > 0.7:
            save_fig(support_train_batch['inp'][0], 'train_img')
            save_fig(support_train_batch['gt'][0], 'train_mask')
            for j in range(2):
                print('j ', j)
                if i % 2 == 0:
                    ...
                    save_fig(query_train_batch['inp'][j], f'test_img_{j + 1 + 2}')
                    save_fig(query_train_batch['gt'][j], f'test_mask_{j + 1 + 2}')
