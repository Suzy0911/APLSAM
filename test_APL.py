...
def save_fig(tensor, name):
    ...
    img.save(os.path.join('save/imgs', name + appendix))
...
def eval_psnr(support_train_batch, query_train_batch, model, i_outer, data_norm=None, eval_type=None, eval_bsize=None,
              verbose=False):
    global config
    model.eval()
    ...
    inp = query_train_batch['inp'].to(device)
    gt = query_train_batch['gt'].to(device)
    support_mask = support_train_batch['gt'].to(device)
    support_img = support_train_batch['inp'].to(device)

    pred = torch.sigmoid(model(inp, gt, support_img, support_mask))

    # model.cpu()
    # torch.cuda.empty_cache()
    # del model
    # print('here 1 97')
    gt_ = torch.where(gt > 0, 1.0, 0.0)
    result1, result2, result3, result4 = metric_fn(pred, gt_)
...
def train(support_train_batch, query_train_batch, model, loss_mode, epoch):
    global config
    model.train()
    inp = query_train_batch['inp'].to(device)  # input
    gt = query_train_batch['gt'].to(device)
    support_mask = support_train_batch['gt'].to(device)  # ground truth
    support_img = support_train_batch['inp'].to(device)
    # max_val = support_mask.max()
    # gt = (support_mask == max_val)  # important here
    # gt = gt * support_mask
    model.optimizer.zero_grad()
    pred = model(inp, gt, support_img, support_mask)
    if loss_mode == 'bce':
        ...
        criterionBCE = torch.nn.BCEWithLogitsLoss()
    gt_ = torch.where(gt > 0, 1.0, 0.0)
    batch_loss = criterionBCE(pred, gt_)
    # print('这里')
    if loss_mode == 'iou':
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config',
                        default="configs/test_apl_sam.yaml")  # /root/SAM_Adapter_MAML/configs/test.yaml
    parser.add_argument('--model',
                        default='save/LiFeO4_50_5_00005_layer4/model_epoch_best_dice.pth')  # 权重保存路径/root/autodl-tmp/model_epoch_best_ber.pth
    ...
    task = 'LiFeO4'
    config['train_dataset']['dataset']['args']['root_path_1'] = os.path.join(data_path, task, 'support_train', 'img')
    config['train_dataset']['dataset']['args']['root_path_2'] = os.path.join(data_path, task, 'support_train', 'mask')
    ...
    support_train_loader, query_train_loader, support_eval_loader, query_eval_loader = make_data_loaders(
        config)  # 这里的eval是真正的evaluation_loader

    dice_recort = []
    total_data_len = len(os.listdir(os.path.join(data_path, task, 'query_train', 'img')))
    for i, (support_train_batch, query_train_batch, support_eval_batch, query_eval_batch) in (
            enumerate(zip(support_train_loader, query_train_loader, support_eval_loader, query_eval_loader))):
        torch.cuda.empty_cache()
        ...
        epochs = 40  # config['train_dataset']['epochs']
        loss_mode = config['model']['args']['loss']
        min_val_v = 0
        for epoch in range(epochs):  # 每个one-shot样本更新几步
            print(f'epoch: {epoch + 1}')
            loss_log = train(support_train_batch, query_train_batch, model, loss_mode, epoch)  # 训练
            print(f'loss:{loss_log}')
            lr_scheduler.step()  # 余弦退火学习率
        metric1, metric2, metric3, metric4 = eval_psnr(support_eval_batch, query_eval_batch, model, i,
                                                       data_norm=config.get('data_norm'),
                                                       eval_type=config.get('eval_type'),
                                                       eval_bsize=config.get('eval_bsize'),
                                                       verbose=True)  # evaluation

        if metric1 >= 0.3:  # 否则是病人间错位support和query没匹配上
            dice_recort.append(metric1)
        print(f'dice : {metric1}')
        writer.add_scalars('percentage', {'val': ((i + 1) * 2 / total_data_len) * 100}, i + 1)
        print(f'percentage: {((i + 1) * 2 / total_data_len) * 100}; total_data_len: {total_data_len}')
        if len(dice_recort)>0:
            writer.add_scalars('DSC', {'val': mean(dice_recort)}, i + 1)
        else:
            writer.add_scalars('DSC', {'val': 0}, i + 1)
        if metric1 > 0.9:  # or metric1 > 0.7:
            save_fig(support_train_batch['inp'][0], 'train_img_1')
            save_fig(support_train_batch['gt'][0], 'train_mask_1')
            # save_fig(support_train_batch['inp'][2], 'train_img_2')
            # save_fig(support_train_batch['gt'][2], 'train_mask_2')
            for j in range(1):
                print('j ', j)
                save_fig(query_train_batch['inp'][j], f'test_img_{j + 1}')
                save_fig(query_train_batch['gt'][j], f'test_mask_{j + 1}')
