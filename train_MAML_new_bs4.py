...
from test_LARGE_new import save_fig,save_fig_pred
import numpy as np
...
def eval_psnr(support_eval_loader, query_eval_loader, model, eval_type=None):
    ...
    with torch.no_grad():
        for i, (support_batch, query_batch) in enumerate(zip(support_eval_loader, query_eval_loader)):
            ...
            support_img = support_batch['inp'].to(device)

            pred = torch.sigmoid(model(inp, batch_gt, support_img, support_mask))

            batch_gt = torch.where(batch_gt>0, 1.0, 0.0)

            ###########save prediction figs##############
            # input_cpu = inp.cpu()
            # gth_cpu = batch_gt.cpu()
            # s_mask_cpu = support_mask.cpu()
            # s_img_cpu = support_img.cpu()
            # predt_cpu = pred.cpu()
            # save_fig(s_mask_cpu[0], f'eval_supp_mask_i_{i}')
            # for n in range(len(inp)):
            #     save_fig(input_cpu[n], f'eval_query_img_i_{i}_{n+1}')
            #     save_fig(gth_cpu[n], f'eval_query_gt_i_{i}_{n+1}')
            #     save_fig_pred(predt_cpu[n], f'eval_query_pred_i_{i}_{n+1}')
...
def train(all_task_list, support_train_loader_dict, query_train_loader_dict, support_eval_loader_dict,
    ...
    model.train()
    inner_epochs = config['train_dataset']['inner_epochs']
    task_batch_index = 0
    task_loss_list = []
    task_batch_size = config['train_dataset']['task_batch_size']
        ...
        for i, task in enumerate(train_task_list):
            ...
            for inner_epoch in range(inner_epochs):
                for j, (support_batch, query_batch) in enumerate(zip(support_train_loader, query_train_loader)):
                    ...
                    pred = model_inner(inp, gt, support_img, support_mask)
                    if loss_mode == 'bce':
                    ...
                    # inp_cpu = inp.cpu()  # TODO:for debug
                    # gt_cpu = gt.cpu()
                    # support_mask_cpu = support_mask.cpu()
                    # pred_cpu = pred.cpu()
                    # save_fig(support_mask_cpu[0], task + f'_train_support_mask_j_{j}')
                    # # embed(header = '00000000000000000000000' )
                    # for m in range(len(inp)):
                    #     save_fig(inp_cpu[m], task + f'_train_query_img_j_{j}_{m+1}')
                    #     save_fig(gt_cpu[m], task + f'_train_query_gt_j_{j}_{m+1}')
                    #     save_fig_pred(pred_cpu[m], task + f'_train_query_pred_j_{j}_{m + 1}')
            ...
            for k, (support_batch, query_batch) in enumerate(zip(support_eval_loader, query_eval_loader)):
                ...
                # pred_cpu = pred.cpu()  # For Debug
                # for m in range(len(inp)):
                #     save_fig(pred_cpu[m], task + f'_train_query_pred_k_{k}_{m + 1}')
                ...
                if loss_mode == 'iou':
                    batch_loss += _iou_loss(pred, gt)
                # (batch_loss / len(eval_loader)).backward()
                batch_loss.backward()
            ...
            for param_model, param_inner_model in zip(model.parameters(), model_inner.parameters()):
                if param_inner_model.grad is not None:
                    if param_model.grad is None:
                        param_model.grad = param_inner_model.grad / len(train_task_list)
                    else:
                        param_model.grad += param_inner_model.grad / len(train_task_list)

            del model_inner  # delete inner model
...
def main(config_, save_path, args):
    ...
    model.load_state_dict(sam_checkpoint, strict=False)  # download weights
    ...
    all_task_list = os.listdir(config['data_path'])  # all tasks
    eval_task = 'LiCoO2'  # TODO:val_loader
    exclude_task_list = [eval_task, 'PS', '__MACOSX', '.DS_Store']
    ...
    support_eval_loader = make_data_loader(config.get('test_dataset'), tag='test')  # for eval_psnr
    query_eval_loader = make_data_loader(config.get('query_eval_dataset'), tag='test')  # for eval_psnr
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', default="./configs/try_ssm-spm.yaml")
    ...
    if save_name is None:
        save_name = '_' + args.config.split('/')[-1][:-len('.yaml')]  # without name
